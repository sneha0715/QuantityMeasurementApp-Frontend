...
export function Subtract() {
  ...
  const subtractMutation = useMutation({
    ...
    onError: (error: unknown) => {
      setApiError(
        getApiErrorMessage(error, "Subtraction failed. Please try again.")
      )
    },
  })
    ...
    <div className="mx-auto max-w-5xl space-y-6">
      ...
      <button
        ...
        disabled={
          ...
          thisMeasurementType === "TemperatureUnit"
        }
      ...
      >
        {subtractMutation.isPending ? "Subtracting..." : "Subtract"}
      </button>

      {result && <ResultCard result={result} operation="subtract" />}
    </div>
  )
}
